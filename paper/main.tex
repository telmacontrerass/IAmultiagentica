...
hierarchy of fine-tuned small-model experts \citep{fu2024camphor}. Each is a
...
\citep{lin2024hammer,rabinovich2025robustness}. The very models a local-first
...
\begin{enumerate}
  ...
  \item \textbf{Deterministic, non-LLM verification.} Agent claims are checked
    ...
    (Sec.~\ref{sec:system-grounding}) --- a choice supported by evidence that
    LLM judges detect false success poorly, keying on confident language rather
    than verified state \citep{advani2026falsesuccess}.
...
interaction \citep{fu2024camphor}. ReST-style self-improvement distils a
prompted ReAct agent into a model two orders of magnitude smaller. Hammer trains
on-device models with function masking to resist misleading tool names
\citep{lin2024hammer}. A recent NVIDIA position paper argues the endpoint of this
...
\citep{wang2024codeact}. Agentless makes the inverse point: a fixed pipeline can
outperform an autonomous agent, so the \emph{structure}, not the agency, carries
the result \citep{xia2024agentless}. At the meta level, Kapoor et al. show that
...
\emph{orthogonal and insufficient}. It solves the syntax of a single call. It
does not decide which tool to call, detect a loop, cut off a repeating error,
budget the context, or determine whether an agent that claims success actually
succeeded. Moreover, at the model scale a local-first system must live with,
forcing validity appears to trade one failure for a worse one: recent evidence at
sub-3B scale reports that mandatory schema conformance \emph{raised}
valid-but-wrong outputs from 49.5\% to 88.9\%, converting visibly broken output
into silently incorrect output \citep{ray2026constrainttax}. That is an argument
for harness-side repair and independent verification, not against constraint.
Accordingly we treat constrained decoding as a \textbf{baseline arm to be
measured} (Sec.~\ref{sec:conditions}), not as a rival to be dismissed.

\subsection{Verification, false success, and the choice of oracle}

Agents over-claim. Sycophancy in RLHF-trained models is a documented root cause
of assistants that tell users what they expect to hear
\citep{sharma2024sycophancy}, and recent work characterises the agentic
consequence directly: \emph{false success}, where an agent reports completion
that programmatic environment state refutes, accounts for a large fraction of
all failures \citep{advani2026falsesuccess}, while outcome-only grading conceals
procedural violations behind apparently successful runs
\citep{cao2026corruptsuccess}. We claim no novelty for the phenomenon --- it is
named and measured in that work --- but we adopt its measurement as a
first-class metric of a harness, alongside task success.

The same work supplies our strongest argument for how to measure it. LLM judges
detect false success poorly (AUROC $\leq 0.65$) precisely because they key on
confident language rather than verified state changes
\citep{advani2026falsesuccess}. Combined with the known biases of
LLM-as-a-judge \citep{zheng2023judging} and the self-preference effect that
arises when judge and agent share a model family
\citep{panickssery2024llmevaluators} --- the normal situation in a single-model
local deployment --- we reject the LLM judge for our core suite and grade by
deterministic oracle. Our grounding gate follows the attribution standard that a
claim counts only if it is verifiable against an independent source
\citep{rashkin2023attribution}, and our permission engine follows the precedent
of deterministic, out-of-model privilege control
\citep{shi2025progent,ruan2024toolemu}.
...
\begin{description}\itemsep2pt
  ...
  \item[A --- Mechanism ablation (the core contribution).] Each scaffolding
    ...
    compaction ladder, and nudges. Given evidence that scaffolding components can
    interfere destructively \citep{liu2026moreisnotbetter}, we also report the
    all-off condition, so that the mechanisms' joint effect can be compared with
  ...
  \item[C --- Constrained-decoding baseline.] A condition in which tool calls
    ...
    Following \citet{ray2026constrainttax}, we report schema validity and task
    correctness \emph{separately}, since constraint is expected to improve the
    former while potentially harming the latter.
...
oracle refuted it. This measures the failure mode that matters most for a local
agent operating unattended --- and, following \citet{advani2026falsesuccess}, it
is measured against environment state, not against a judge's reading of the
agent's prose.
...
constrained-decoding row is the one we are least able to predict; per
\citet{ray2026constrainttax}, we specifically test whether it raises schema
validity while \emph{worsening} false success.
