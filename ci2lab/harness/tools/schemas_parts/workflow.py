"""OpenAI function schemas for workflow-control tools.

Defines :data:`WORKFLOW_SCHEMAS`, the schemas for tools that orchestrate the
agent's own workflow (delegating subtasks, managing the todo list, asking the
user). These are aggregated into the full tool set by
...
#: OpenAI-compatible function schemas for workflow-control tools.
