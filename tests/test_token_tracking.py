...
def test_chat_captura_usage(monkeypatch):
    ...
    assert resp.usage == {"prompt_tokens": 42, "completion_tokens": 3, "total_tokens": 45}
...
def test_stream_chat_captura_usage(monkeypatch):
    ...
    assert final.usage == {"prompt_tokens": 55, "completion_tokens": 7, "total_tokens": 62}
...
def test_stream_chat_usage_en_chunk_separado(monkeypatch):
    ...
    final = next(e for e in events if isinstance(e, LLMResponse))
    assert final.usage == {"prompt_tokens": 10, "completion_tokens": 2, "total_tokens": 12}
