...
| 7 | Typo/modelo ausente → crash críptico (o modelo equivocado en silencio) | (a) bug de parser `--model` antes del subcomando; (b) check pre-vuelo con error claro; (c) "did you mean" — **sin auto-pull** | [`ci2lab/cli/parser.py`](ci2lab/cli/parser.py), [`ci2lab/runtime/preflight.py`](ci2lab/runtime/preflight.py), [`ci2lab/router/catalog.py`](ci2lab/router/catalog.py), [`ci2lab/cli/commands/agent.py`](ci2lab/cli/commands/agent.py) | ✅ Hecho: parser con `argparse.SUPPRESS` (ya no pisa `--model`), `check_model_available()` (error claro, cero descarga), `suggest_similar_models()`, cableado en `_run_turn`/`_run_repl`; con tests |
...
| 4 | Parsing frágil | 🟡 **A medias — y en su punto de máximo valor razonable.** Hecho: (a) **validación de argumentos requeridos** ausentes → mensaje accionable `outcome="invalid_arguments"` en vez del `KeyError` críptico ([`executor_parts/arguments.py`](ci2lab/harness/tools/executor_parts/arguments.py) + enganche en [`core.py`](ci2lab/harness/tools/executor_parts/core.py)); (b) **coerción de booleanos stringy** (`"false"`→`False`; arreglaba un *bug silencioso de corrección*); (c) **coerción de enteros stringy** (`"2"`→`2`, schema-driven; tapaba el hueco de `tree`/`inspect_file` que fallaba críptico); (d) "más formatos de parser" ya estaba cubierto (native/XML/fenced/JSON). **Resto = OPCIONAL, no afecta a los pain points (deferido sin evidencia):** coerción JSON-a-estructura (recuperar un `array`/`object` doble-codificado como string) + validación de tipos con rechazo. Es cola larga rara, sin bug silencioso escondido (lo frecuente y lo peligroso ya está hecho). Documentado por si en el futuro apetece; **solo tocar si aparece en runs reales / benchmarks**. De momento, no. |
